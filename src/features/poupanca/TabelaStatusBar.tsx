...
export function TabelaStatusBar({ nnm, meta, metaMensal, tombamento }: Props) {
  ...
  const corTexto = pctRaw >= 100 ? '#16a34a' : pctRaw > 0 ? '#ca8a04' : '#dc2626';
  ...
  return (
    <div title={titulo} className="w-full">
      ...
      <p className="mt-0.5 text-center" style={{ fontSize: 11, fontWeight: 700, color: corTextoFinal }}>
        {tombamento ? badge : temMeta ? `${pctLabel}%` : badge}
      </p>
    </div>
  );
}
